...
class ComparisonWindow:
    ...
    def create_time_controls(self):
        ...
        # Buttons
        ttk.Button(entry_frame, text="Update", command=self.update_plot).pack(side=tk.LEFT, padx=5)
    ...
    def update_plot(self):
        ...
        try:
            ...
            if self.plot_type in ["Current", "Power"] and self.all_phases_var.get():
                ...
                power_values = plot_data[f'total'].values
                energy = np.sum((power_values[:-1] + power_values[1:]) / 2 * time_diff) / 3600000  # Convert to kWh
                # Update max et min label values 
                self.max_value_var.set(f"Max: {float(np.max(power_values)):.2f}")
                self.min_value_var.set(f"Min: {float(np.min(power_values)):.2f}")
                if self.plot_type == "Power":
...
class PowerMonitorApp:
    ...
    def create_display_options(self, parent):
        ...
        # Phase windows selection
        phase_frame = ttk.Frame(options_frame)
    ...
    def create_phase_window(self, phase_num, data):
        ...
        # Buttons
        ttk.Button(entry_frame, text="Update", command=update_plots).pack(side=tk.LEFT, padx=5)
