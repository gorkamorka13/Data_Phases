...
# - Ajout des Min et Max aux fenetres Comparaison
# - Ajout des Min et Max aux fenetre de phases
...
class ComparisonWindow:
    ...
    def create_time_controls(self):
        ...
        ttk.Label(visibility_frame, text="Show/Hide Phases:", font=('Arial', 10, 'bold')).pack(side=tk.LEFT, padx=5)
        ...
        #Pour les plot-type "Power et current" création d'une checkbox "All phases"
        if self.plot_type in ["Current", "Power"]:
            self.add_all_phases_checkbox = ttk.Checkbutton(
                visibility_frame, 
                text="All Phases", 
                variable=self.all_phases_var
            )
            self.add_all_phases_checkbox.pack(side=tk.LEFT, padx=5, pady=5) 
            color_label = ttk.Label(visibility_frame, text="■", foreground="purple")            
            color_label.pack(side=tk.LEFT, padx=(0, 20))  
            
            
        #BDébut de création des labels de données (energie, max, min)
    ...
    def get_column_names(self):
        ...
        if self.plot_type == "Voltage":
            return ["voltagemoy1", "voltagemoy2", "voltagemoy3"]
        elif self.plot_type == "Current":
            return ["currentmoy1", "currentmoy2", "currentmoy3"]
        else:  # Power
            return ["powermoy1", "powermoy2", "powermoy3"]
    ...
    def update_filtered_data(self):
        ...
        selected_phases = [phase for phase, var in self.phase_visibility.items() if var.get()]
        
        # Get the filter columns based on the selected phases
        columns = [self.get_filter_column(phase=phase[-1]) for phase in selected_phases]
    ...
    def update_plot(self):
        ...
        try:
            ...
            if self.plot_type in ["Current", "Power"] and self.all_phases_var.get():
                # Calculate the sum of the three phases data
                if self.plot_type == "Current":
                    plot_data['total'] = (
                        plot_data['currentmoy1'] +
                        plot_data['currentmoy2'] +
                        plot_data['currentmoy3']
                    )
                else:  # Power
                    plot_data['total'] = (
                        plot_data['powermoy1'] +
                ...
                total_values = plot_data[f'total'].values
                energy = np.sum((total_values[:-1] + total_values[1:]) / 2 * time_diff) / 3600000  # Convert to kWh
                
                # Update max et min label values          
                max_idx = plot_data[f'total'].idxmax()  # This returns the index label
                ...
                self.max_value_var.set(f"Max: {float(plot_data[f'total'].max()):.2f}"+f" ({datetime_str})")                            
                min_idx = plot_data[f'total'].idxmin()  # This returns the index label
                ...
                self.min_value_var.set(f"Min: {float(plot_data[f'total'].min()):.2f}"+f" ({datetime_str})")   
                                  

                if self.plot_type == "Power":
                    self.energy_var.set(f"Energy: {energy:.2f} kWh")
                                    
                # Plot the total data over time
                self.ax.plot(plot_data['time'], plot_data['total'], label='Total', color='purple')
                self.ax.set_title(f'Total {self.plot_type} Over Time')

                # Calculate and display average
                avg_value = plot_data['total'].mean()

                # Add horizontal line for average
                self.ax.axhline(y=avg_value, color='purple', linestyle='--', alpha=0.5)

                # Add text annotation for average
                self.ax.annotate(
                    f'Avg Total: {avg_value:.2f}',
                    xy=(0.02, 0.98),  # Position text at top-left
                ...
                for col, color, label in zip(columns, colors, labels):
                    if self.phase_visibility[label].get():  # Only plot if phase is visible
                        ...
                        # Met à jour les labels Max and Min                                
...
class PowerMonitorApp:
    ...
    def create_phase_window(self, phase_num, data):
        ...
        ttk.Label(avg_frame, textvariable=voltage_avg_var, font=('Arial', 10, 'bold')).pack(side=tk.LEFT, padx=20)
        ttk.Label(avg_frame, textvariable=current_avg_var, font=('Arial', 10, 'bold')).pack(side=tk.LEFT, padx=20)
        ttk.Label(avg_frame, textvariable=power_avg_var, font=('Arial', 10, 'bold')).pack(side=tk.LEFT, padx=20)
        ttk.Label(avg_frame, textvariable=energy_var, font=('Arial', 10, 'bold')).pack(side=tk.LEFT, padx=20)        
        ...
        ttk.Label(avg_frame, textvariable=voltage_max_var, font=('Arial', 10, 'bold')).pack(side=tk.LEFT, padx=20)
        ttk.Label(avg_frame, textvariable=current_max_var, font=('Arial', 10, 'bold')).pack(side=tk.LEFT, padx=20)
        ttk.Label(avg_frame, textvariable=power_max_var, font=('Arial', 10, 'bold')).pack(side=tk.LEFT, padx=20)        
